...
class Cube extends GlComponent {
    constructor({ gl, shaders, points, pointSize, color, colors, indices }) {
        super({
            gl: gl,
            shaders: shaders,
            buffers: {
    ...
    draw() {
        this.useProgram(this.currentShader)
        this.setupAttribute("aPosition", "vboPosition", 3, this.gl.FLOAT, false, 0, 0)
        this.buffers["ibo"].loadBufferElementData(this.indices);
        ...
        this.gl.drawElements(primitive, this.indices.length, gl.UNSIGNED_SHORT, 0);
    }
}